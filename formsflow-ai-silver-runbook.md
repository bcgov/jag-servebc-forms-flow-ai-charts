...
2. **Hardcoded `runAsUser: 1001` on a custom init container breaks OpenShift's SCC.** `charts/forms-flow-idm/values.yaml`'s `keycloak.initContainers` included a custom `formsflow-themes` container (pulls `formsflow/keycloak-customizations:v7.3.0` to seed themes/providers/realm-import files) with `securityContext.runAsUser: 1001` hardcoded. OpenShift's `restricted`/`restricted-v2` SCC only allows this namespace's assigned UID range (confirmed error: `runAsUser: Invalid value: 1001: must be in the ranges: [1011120000, 1011129999]`), so the Keycloak StatefulSet couldn't schedule at all — `0/1` forever, no SCC matched. The chart's *main* Keycloak container already does this correctly (`containerSecurityContext.enabled: false` / `podSecurityContext.enabled: false`, letting OpenShift auto-assign); this one custom init container just didn't follow that pattern.

   **Fixed directly in the chart source (2026-08-18)**, not with an install-time override — this isn't environment-specific, it's wrong for *any* OpenShift-restricted-SCC deployment, and this repo exists specifically to deploy on OpenShift. Deleted the `securityContext:`/`runAsUser: 1001` lines from `charts/forms-flow-idm/values.yaml`'s `formsflow-themes` init container entry outright. Confirmed live: `helm upgrade` (no `-f` override needed) rolled the StatefulSet to a new revision, and OpenShift's admission controller auto-injected the correct `runAsUser: 1011120000` (this namespace's actual assigned UID, from the allowed range above) into the running pod — exactly the same effect the earlier values-override-file workaround achieved, now built in. That earlier workaround (`deploy-overrides/forms-flow-idm-overrides.yaml`) has been deleted; don't recreate it.

   **⚠️ If this chart is ever re-synced from AOT's upstream `forms-flow-ai-charts` (see Phase -1's `git fetch upstream` maintenance pattern), check whether `charts/forms-flow-idm/values.yaml`'s `formsflow-themes` init container has a `securityContext.runAsUser` again before merging** — a straight upstream merge would silently reintroduce this exact bug (upstream's chart isn't OpenShift-SCC-aware; this fix is local-only, not something AOT will carry). Delete it again the same way if it reappears. (For the record — targeting just the `securityContext` field via `--set-json 'keycloak.initContainers[0].securityContext={}'` at install time, instead of editing the chart, silently wiped every *other* field of that same array element, `name`/`image`/`command`/`args`/`volumeMounts` all disappearing from the rendered manifest — a Helm gotcha with `--set` on array indices; if a future non-source fix is ever needed again for some reason, use a full values-override file, not a partial `--set`.)
